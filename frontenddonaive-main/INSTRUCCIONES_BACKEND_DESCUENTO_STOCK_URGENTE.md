# 🚨 INSTRUCCIONES URGENTES BACKEND: Descuento de Stock/Existencia al Confirmar Venta
...
**Situación actual:**
- Usuario vende 1 producto que tiene stock 2
- Después de la venta, el stock sigue mostrando 2 (NO se descuenta)
- En el inventario aparece como "existencia"
- En el punto de venta aparece como "stock"
- **AMBOS deben descontarse cuando se confirma la venta**
...
**Campos a actualizar:**
- `cantidad` (en el array de items del inventario)
- `existencia` (debe ser igual a cantidad)
- `stock` (debe ser igual a cantidad)

**IMPORTANTE:** Estos tres campos deben mantenerse sincronizados y todos deben descontarse.
...
### PASO 2: Procesar Cada Item y Descontar Stock/Existencia
    ...
    async with await db.client.start_session() as session:
        async with session.start_transaction():
            ...
            for item_venta in venta_data.items:
                ...
                # Obtener stock actual (verificar múltiples campos)
                stock_actual = (
                    item_encontrado.get("cantidad") or 
                    item_encontrado.get("existencia") or 
                    item_encontrado.get("stock") or 
                    0
                )
                ...
                # Actualizar TODOS los campos relacionados para mantener consistencia
                ...
                # Log para depuración
                print(f"✅ [VENTA] Producto descontado: {item_venta.nombre}")
...
5. **Actualizar TODOS los campos relacionados** (`cantidad`, `existencia`, `stock`) para mantener consistencia
...
print(f"✅ [VENTA] Producto encontrado en inventario.")
print(f"   - Stock actual (cantidad): {item_encontrado.get('cantidad')}")
print(f"   - Stock actual (existencia): {item_encontrado.get('existencia')}")
print(f"   - Stock actual (stock): {item_encontrado.get('stock')}")
print(f"   - Cantidad a descontar: {cantidad_vendida}")
print(f"   - Nuevo stock: {nueva_cantidad}")

# Después de actualizar:
print(f"✅ [VENTA] Inventario actualizado exitosamente.")
print(f"   - Nuevo stock total (suma de todos los items): {sum(item.get('cantidad', 0) for item in items)}")
```

---

## ⚠️ CAMPOS A ACTUALIZAR (CRÍTICO)

Cuando se descuenta el stock, DEBES actualizar estos tres campos para mantener consistencia:

```python
# Después de calcular nueva_cantidad:
items[item_index]["cantidad"] = nueva_cantidad      # ✅ CRÍTICO
items[item_index]["existencia"] = nueva_cantidad    # ✅ CRÍTICO
items[item_index]["stock"] = nueva_cantidad         # ✅ CRÍTICO
```

**Razón:** 
- En el inventario se muestra como "existencia"
- En el punto de venta se muestra como "stock"
- Ambos deben estar sincronizados con "cantidad"

---

## 🧪 TESTING RECOMENDADO

1. **Test con stock suficiente:** 
   - Producto con stock 2, vender 1
   - Verificar que el stock quede en 1
   - Verificar que `cantidad`, `existencia` y `stock` estén todos en 1

2. **Test con stock insuficiente:** 
   - Producto con stock 1, intentar vender 2
   - Verificar que lanza error y NO descuenta nada

3. **Test con producto no encontrado:** 
   - Intentar vender producto que no existe en inventario
   - Verificar que lanza error 404

4. **Test con múltiples productos:** 
   - Vender 3 productos diferentes
   - Verificar que todos se descuenten correctamente

5. **Test de transacción:** 
   - Si falla algo, verificar que NO se descuenta nada (atomicidad)
...
async def crear_venta(
    ...
    CRÍTICO: Debe actualizar cantidad, existencia y stock.
    """
    try:
        # Usar transacción
        async with await db.client.start_session() as session:
            async with session.start_transaction():
                ...
                for item_venta in venta_data.items:
                    ...
                    # ⚠️ CRÍTICO: Actualizar los tres campos
                    items[item_index]["cantidad"] = nueva_cantidad
                    items[item_index]["existencia"] = nueva_cantidad
                    items[item_index]["stock"] = nueva_cantidad
                    items[item_index]["fecha_actualizacion"] = datetime.now()
...
3. **Actualizar TODOS los campos relacionados** (`cantidad`, `existencia`, `stock`) para mantener consistencia
...
1. ✅ Vender un producto con stock 2
2. ✅ Verificar que el stock quede en 1 (en inventario y punto de venta)
3. ✅ Verificar que `cantidad`, `existencia` y `stock` estén todos en 1
4. ✅ Verificar que el punto de venta muestre el stock actualizado al buscar productos
